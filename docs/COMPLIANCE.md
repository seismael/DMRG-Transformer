...
| Stacked TTBlock real-task validation | ✅ | [scripts/train_real_world_tt_block_classifier.py](../scripts/train_real_world_tt_block_classifier.py) → [bench/REAL_WORLD_TT_BLOCK.md](../bench/REAL_WORLD_TT_BLOCK.md). TT-DMRG runs full Q/K/V/W_out + FFN updates (trust-region accept/revert on Q,K bilinear path) **plus** exact-LSQ input-projection updates (also trust-region wrapped) and reaches ~0.72 test acc on sklearn-digits-as-sequence vs Adam ~0.88 (~16 pp residual gap, root-caused to pooled-target broadcast + late-epoch trust-region rejections — see bench file's *Honest gap analysis* section). Regression-guarded by [tests/test_real_world_tt_block_classifier.py](../tests/test_real_world_tt_block_classifier.py). |
